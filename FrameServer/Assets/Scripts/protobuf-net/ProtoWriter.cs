...
namespace ProtoBuf
{
    ...
    public sealed class ProtoWriter : IDisposable
    {
        ...
        public static void WriteObject(object value, int key, ProtoWriter writer)
        {
            ...
            else if (writer.Model != null && writer.Model.TrySerializeAuxiliaryType(writer, value.GetType(),
                         DataFormat.Default, Serializer.ListItemTag, value, false))
            {
                // all ok
            }
            else
        ...
        internal static void WriteObject(object value, int key, ProtoWriter writer, PrefixStyle style, int fieldNumber)
        {
            ...
            if (key < 0)
            {
                if (!writer.Model.TrySerializeAuxiliaryType(writer, value.GetType(), DataFormat.Default,
                        Serializer.ListItemTag, value, false)) TypeModel.ThrowUnexpectedType(value.GetType());
            }
            else
            {
                writer.Model.Serialize(key, value, writer);
        ...
        public static void WriteFieldHeader(int fieldNumber, WireType wireType, ProtoWriter writer)
        {
            ...
            if (writer.WireType != WireType.None)
                throw new InvalidOperationException("Cannot write a " + wireType
                                                                      + " header until the " + writer.WireType +
            ...
            else
            {
                throw new InvalidOperationException("Field mismatch during packed encoding; expected " +
                                                    writer.packedFieldNumber + " but received " + fieldNumber);
            }
        }
...
#endif
                throw new ProtoException("Possible recursion detected (offset: " + (recursionStack.Count - hitLevel) +
                                         " level(s)): " + instance);
            }
        ...
        public static void ThrowEnumException(ProtoWriter writer, object enumValue)
        {
            ...
            throw new ProtoException("No wire-value is mapped to the enum " + rhs + " at position " + writer.position);
        }
        ...
        {
            if (writer == null) throw new ArgumentNullException("writer");
            return new ProtoException("Invalid serialization operation with wire-type " + writer.WireType +
                                      " at position " + writer.position);
        }
