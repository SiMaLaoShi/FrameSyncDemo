...
namespace ProtoBuf
{
    ...
    public abstract class Extensible : IExtensible
    {
        ...
        {
            if (createIfMissing && extensionObject == null) extensionObject = new BufferExtension();
            ...
            bool allowDefinedTag, out object value)
        {
            value = null;
            var set = false;
            foreach (var val in ExtensibleUtil.GetExtendedValues(model, type, instance, tag, format, true,
                         allowDefinedTag))
            {
        ...
        public static IEnumerable GetValues(TypeModel model, Type type, IExtensible instance, int tag,
            DataFormat format)
        {
