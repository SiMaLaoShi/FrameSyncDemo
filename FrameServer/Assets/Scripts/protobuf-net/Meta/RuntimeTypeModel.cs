    ...
    public sealed class RuntimeTypeModel : TypeModel
    {
        ...
        public override string GetSchema(Type type)
        {
            ...
            else
            {
                ...
                isInbuiltType = ValueMember.TryGetCoreSerializer(this, DataFormat.Default, type, out defaultWireType,
            ...
            if (isInbuiltType)
            {
                ...
                MetaType.NewLine(bodyBuilder, 1).Append("optional ")
                ...
                if (metaType.IsAutoTuple)
                {
                    ...
                    if (MetaType.ResolveTupleConstructor(metaType.Type, out mapping) != null)
                        for (var i = 0; i < mapping.Length; i++)
                        {
                            ...
                            WireType defaultWireType;
                            var coreSerializer = ValueMember.TryGetCoreSerializer(this, DataFormat.Default, type,
                                out defaultWireType, false, false, false, false);
                            if (coreSerializer == null)
                            {
                                var index = FindOrAddAuto(type, false, false, false);
        ...
        internal IProtoSerializer TryGetBasicTypeSerializer(Type type)
        {
            ...
            lock (basicTypes)
            {
                ...
                var ser = family == MetaType.AttributeFamily.None
                    ? ValueMember.TryGetCoreSerializer(this, DataFormat.Default, type, out defaultWireType, false,
                        false, false, false)
                    : null;
            ...
            if (key < 0)
            {
                ...
                try
                {
                    ...
                    if ((metaType = RecogniseCommonTypes(type)) == null)
                    {
                        ...
                        if (!shouldAdd || !Helpers.IsEnum(type) && addWithContractOnly &&
                            family == MetaType.AttributeFamily.None
                           )
                        {
...
//#endif
        ...
        public sealed class CompilerOptions
        {
            ...
            /// </summary>
            public string TargetFrameworkName { get { return targetFrameworkName; } set { targetFrameworkName =
 value; } }

            /// <summary>
            /// The TargetFrameworkAttribute FrameworkDisplayName value to burn into the generated assembly
            /// </summary>
            public string TargetFrameworkDisplayName { get { return targetFrameworkDisplayName; } set { targetFrameworkDisplayName
 = value; } }
            /// <summary>
            ...
            /// </summary>
            public string ImageRuntimeVersion { get { return imageRuntimeVersion; } set { imageRuntimeVersion =
 value; } }
            /// <summary>
            /// The runtime version for the generated assembly
            /// </summary>
...
#pragma warning restore 0618
 ...
 WriteSerializeDeserialize(assemblyName, type, methodPairs, ilVersion, ref il);
        ...
        private Compiler.CompilerContext WriteSerializeDeserialize(string assemblyName, TypeBuilder type, SerializerPair[] methodPairs, Compiler.CompilerContext.ILVersion ilVersion, ref ILGenerator il)
        {
            ...
            Compiler.CompilerContext ctx =
 new Compiler.CompilerContext(il, false, true, methodPairs, this, ilVersion, assemblyName, MapType(typeof(object)), "Serialize " + type.Name);
            ...
            ctx =
 new Compiler.CompilerContext(il, false, false, methodPairs, this, ilVersion, assemblyName, MapType(typeof(object)), "Deserialize " + type.Name);
        ...
        private const int KnownTypes_Array = 1, KnownTypes_Dictionary = 2, KnownTypes_Hashtable =
 3, KnownTypes_ArrayCutoff = 20;
        private void WriteGetKeyImpl(TypeBuilder type, bool hasInheritance, SerializerPair[] methodPairs, Compiler.CompilerContext.ILVersion ilVersion, string assemblyName, out ILGenerator il, out int knownTypesCategory, out FieldBuilder knownTypes, out Type knownTypesLookupType)
        {
            ...
            Compiler.CompilerContext ctx =
 new Compiler.CompilerContext(il, false, false, methodPairs, this, ilVersion, assemblyName, MapType(typeof(System.Type), true), "GetKeyImpl");
            ...
            knownTypes =
 type.DefineField("knownTypes", knownTypesLookupType, FieldAttributes.Private | FieldAttributes.InitOnly | FieldAttributes.Static);
...
,
            ...
            for (index = 0; index < methodPairs.Length; index++)
            {
                ...
                ctx =
 new Compiler.CompilerContext(pair.SerializeBody, true, true, methodPairs, this, ilVersion, assemblyName, pair.Type.Type, "SerializeImpl " + pair.Type.Type.Name);
                ...
                ctx =
 new Compiler.CompilerContext(pair.DeserializeBody, true, false, methodPairs, this, ilVersion, assemblyName, pair.Type.Type, "DeserializeImpl " + pair.Type.Type.Name);
 ...
 (baseType.GetTypeInfo().Attributes & ~TypeAttributes.Abstract) | TypeAttributes.Sealed;
#else
 ...
 GetType("System.Runtime.Versioning.TargetFrameworkAttribute", Helpers.GetAssembly(MapType(typeof(string))));
            ...
            try
            {
                internalsVisibleToAttribType =
 MapType(typeof(System.Runtime.CompilerServices.InternalsVisibleToAttribute));
            }
        ...
        private static MethodBuilder EmitBoxedSerializer(TypeBuilder type, int i, Type valueType, SerializerPair[] methodPairs, TypeModel model, Compiler.CompilerContext.ILVersion ilVersion, string assemblyName)
        {
            ...
            MethodBuilder boxedSerializer =
 type.DefineMethod("_" + i.ToString(), MethodAttributes.Static, CallingConventions.Standard,
 ...
 new Compiler.CompilerContext(boxedSerializer.GetILGenerator(), true, false, methodPairs, model, ilVersion, assemblyName, model.MapType(typeof(object)), "BoxedSerializer "  + valueType.Name);
        ...
        internal void TakeLock(ref int opaqueToken)
        {
            ...
            opaqueToken =
 Interlocked.CompareExchange(ref contentionCounter, 0, 0); // just fetch current value (starts at 1)
#elif CF2 || CF35
 ...
 typeInfo.GenericTypeArguments))
                        ...
                        if (type.IsGenericType && type.GetGenericTypeDefinition() == MapType(typeof(IDictionary<,>))
                                               && itemType == MapType(typeof(KeyValuePair<,>))
            ...
            ref bool requiresBclImport)
        {
            ...
            WireType wireType;
            var ser = ValueMember.TryGetCoreSerializer(this, dataFormat, effectiveType, out wireType, false, false,
                false, false);
            if (ser == null)
            {
                // model type
        ...
        internal void VerifyFactory(MethodInfo factory, Type type)
        {
            if (factory != null)
            {
                ...
                if (!CallbackSet.CheckCallbackParameters(this, factory))
                    throw new ArgumentException(
                        "Invalid factory signature in " + factory.DeclaringType.FullName + "." + factory.Name,
                        "factory");
            }
        }
    }
