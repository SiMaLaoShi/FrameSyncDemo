    ...
    public class MetaType : ISerializerProxy
    {
        ...
        public MetaType AddSubType(int fieldNumber, Type derivedType, DataFormat dataFormat)
        {
            ...
            if (!IsValidSubType(derivedType))
                throw new ArgumentException(derivedType.Name + " is not a valid sub-type of " + type.Name,
                    "derivedType");
        ...
        public MetaType SetCallbacks(MethodInfo beforeSerialize, MethodInfo afterSerialize,
            MethodInfo beforeDeserialize, MethodInfo afterDeserialize)
        {
                ...
                .IsGenericType)
            {
                        ...
                        )
                {
                    ...
                    if (key >= 0 && (mt = model[tmp]) != null &&
                        mt.surrogate == null) // <=== need to exclude surrogate to avoid chance of infinite loop
        ...
        protected internal void ThrowIfFrozen()
        {
            if ((flags & OPTIONS_Frozen) != 0)
                throw new InvalidOperationException(
                    "The type cannot be changed once a serializer has been generated for " + type.FullName);
        }
        ...
        {
            if (Helpers.IsEnum(type))
                return new TagDecorator(ProtoBuf.Serializer.ListItemTag, WireType.Variant, false,
                    new EnumSerializer(type, GetEnumMap()));
            var itemType = IgnoreListHandling ? null : TypeModel.GetListItemType(model, type);
            if (itemType != null)
            {
                if (surrogate != null)
                ...
                var fakeMember = new ValueMember(model, ProtoBuf.Serializer.ListItemTag, type, itemType, defaultType,
                    DataFormat.Default);
                return new TypeSerializer(model, type, new[] { ProtoBuf.Serializer.ListItemTag },
                    new[] { fakeMember.Serializer }, null, true, true, null, constructType, factory);
            }
            ...
            if (subTypeCount != 0)
                foreach (SubType subType in subTypes)
                {
                    ...
                    if (!subType.DerivedType.IgnoreListHandling &&
                        model.MapType(ienumerable).IsAssignableFrom(subType.DerivedType.Type))
#endif
                        throw new ArgumentException(
            ...
            return new TypeSerializer(model, type, fieldNumbers, serializers, arr, BaseType == null, UseConstructor,
                callbacks, constructType, factory);
        }
        ...
        internal void ApplyDefaultBehaviour()
        {
            ...
            for (var i = 0; i < typeAttribs.Length; i++)
            {
                ...
                if (!isEnum && fullAttributeTypeName == "ProtoBuf.ProtoIncludeAttribute")
                {
                    ...
                    if (item.TryGet("DataFormat", out tmp)) dataFormat = (DataFormat)(int)tmp;
                    Type knownType = null;
                    try
                    {
                ...
                if (fullAttributeTypeName == "ProtoBuf.ProtoContractAttribute")
                {
                    ...
                    if (Helpers.IsEnum(
                            type)) // note this is subtly different to isEnum; want to do this even if [Flags]
                    {
#if !FEAT_IKVM
                    ...
                    else
                    {
                        ...
                        if (item.TryGet("ImplicitFields", out tmp) && tmp != null)
                            implicitMode =
                                (ImplicitFields)
                                (int)tmp; // note that this uses the bizarre unboxing rules of enums/underlying-types
            ...
            foreach (var member in foundList)
            {
                ...
                if ((property = member as PropertyInfo) != null)
                {
                    ...
                    isField = false;
                    ApplyDefaultBehaviour_AddMembers(model, family, isEnum, partialMembers, dataMemberOffset,
                        inferTagByName, implicitMode, members, member, ref forced, isPublic, isField,
                        ref effectiveType);
                }
                ...
                else if ((method = member as MethodInfo) != null)
                {
                    ...
                    {
                        CheckForCallback(method, memberAttribs, "ProtoBuf.ProtoBeforeSerializationAttribute",
                            ref callbacks, 0);
                        CheckForCallback(method, memberAttribs, "ProtoBuf.ProtoAfterSerializationAttribute",
                            ref callbacks, 1);
                        CheckForCallback(method, memberAttribs, "ProtoBuf.ProtoBeforeDeserializationAttribute",
                            ref callbacks, 2);
                        CheckForCallback(method, memberAttribs, "ProtoBuf.ProtoAfterDeserializationAttribute",
                            ref callbacks, 3);
            ...
            BasicList members, MemberInfo member, ref bool forced, bool isPublic, bool isField, ref Type effectiveType)
        {
            ...
            {
                var normalizedAttribute = NormalizeProtoMember(model, member, family, forced, isEnum, partialMembers,
        ...
        internal static ConstructorInfo ResolveTupleConstructor(Type type, out MemberInfo[] mappedMembers)
        {
            ...
            for (var i = 0; i < fieldsPropsUnfiltered.Length; i++)
            {
                ...
                if (prop != null)
                {
                    ...
                    if (prop.CanWrite && Helpers.GetSetMethod(prop, false, false) != null)
                        return
                            null; // don't allow a public set (need to allow non-public to handle Mono's KeyValuePair<,>)
                    memberList.Add(prop);
                }
                else
                {
            ...
            for (var i = 0; i < ctors.Length; i++)
            {
                ...
                {
                    if (string.Compare(parameters[j].Name, members[k].Name, StringComparison.OrdinalIgnoreCase) !=
                        0) continue;
                ...
                if (attributes[i].AttributeType.FullName == callbackTypeName)
                {
                    ...
                    else if (callbacks[index] != null)
                    {
                        ...
                        throw new ProtoException(
                            "Duplicate " + callbackTypeName + " callbacks on " + reflected.FullName);
                    }
            ...
            bool inferByTagName)
        {
            ...
            if (!ignore && !done && HasFamily(family, AttributeFamily.DataContractSerialier))
            {
                ...
                if (attrib != null)
                {
                    ...
                    if (done)
                        fieldNumber +=
                            dataMemberOffset; // dataMemberOffset only applies to DCS flags, to allow us to "bump" WCF by a notch
                }
            }
        ...
        private ValueMember ApplyDefaultBehaviour(bool isEnum, ProtoMemberAttribute normalizedAttribute)
        {
            ...
            var vm = isEnum || normalizedAttribute.Tag > 0
                ? new ValueMember(model, type, normalizedAttribute.Tag, member, effectiveType, itemType, defaultType,
            ...
            if (vm != null)
            {
                ...
                {
                    var method =
                        Helpers.GetInstanceMethod(finalType, "ShouldSerialize" + member.Name, Helpers.EmptyTypes);
        ...
        public void SetSurrogate(Type surrogateType)
        {
            ...
            if (surrogateType != null)
                ...
                if (surrogateType != null &&
                    Helpers.IsAssignableFrom(model.MapType(typeof(IEnumerable)), surrogateType))
                    throw new ArgumentException(
...
#if WINRT
            mi =
 Helpers.IsEnum(type) ? type.GetTypeInfo().GetDeclaredField(memberName) : Helpers.GetInstanceMember(type.GetTypeInfo(), memberName);
            ...
            var newField = new ValueMember(model, type, fieldNumber, mi, miType, itemType, defaultType,
                ...
                if (defaultType == null)
                {
...
#endif
                    {
                        ...
                        if (typeInfo.IsGenericType && type.GetGenericTypeDefinition() == typeof(System.Collections.Generic.IDictionary<,>)
                            && itemType == typeof(System.Collections.Generic.KeyValuePair<,>).MakeGenericType(genArgs =
 typeInfo.GenericTypeArguments))
#else
                        if (type.IsGenericType && type.GetGenericTypeDefinition() ==
                                               model.MapType(typeof(IDictionary<,>))
                                               && itemType == model.MapType(typeof(KeyValuePair<,>))
        ...
        internal void WriteSchema(StringBuilder builder, int indent, ref bool requiresBclImport)
        {
            ...
            if (IsList)
            {
                var itemTypeName = model.GetSchemaTypeName(TypeModel.GetListItemType(model, type), DataFormat.Default,
            ...
            else if (IsAutoTuple)
            {
                ...
                if (ResolveTupleConstructor(type, out mapping) != null)
                {
                    ...
                    for (var i = 0; i < mapping.Length; i++)
                    {
                        ...
                        NewLine(builder, indent + 1).Append("optional ").Append(model
                                .GetSchemaTypeName(effectiveType, DataFormat.Default, false, false,
            ...
            else if (Helpers.IsEnum(type))
            {
                ...
                else
                {
                    foreach (var member in fieldsArr)
                        NewLine(builder, indent + 1).Append(member.Name).Append(" = ").Append(member.FieldNumber)
                            .Append(';');
                }
            ...
            else
            {
                ...
                foreach (var member in fieldsArr)
                {
                    ...
                    builder.Append(';');
                    if (schemaTypeName == "bcl.NetObjectProxy" && member.AsReference &&
                        !member.DynamicType) // we know what it is; tell the user
