...
namespace ProtoBuf
{
 ...
 typeof(string).GetTypeInfo().Assembly.GetType("System.Runtime.Serialization.FormatterServices");
                    MethodInfo method =
 formatterServiceType?.GetMethod("GetUninitializedObject", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static);
                    if (method != null)
                    {
        ...
        {
            DateTimeKind kind;
            var ticks = ReadTimeSpanTicks(source, out kind);
                ...
                switch (fieldNumber)
                {
                    ...
                    case FieldTypeName:
                        ...
                        else
                        {
                            key = source.GetTypeKey(ref type);
                            if (key < 0)
                                throw new InvalidOperationException("Dynamic type is not a contract-type: " +
                                                                    type.Name);
                        }
