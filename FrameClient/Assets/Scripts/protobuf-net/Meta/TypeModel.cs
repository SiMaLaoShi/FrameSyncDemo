    ...
    public abstract class TypeModel
    {
            ...
            SerializationContext context)
        {
            ...
            try
            {
                ...
                {
                    if (!(TryDeserializeAuxiliaryType(reader, DataFormat.Default, Serializer.ListItemTag, type,
                            ref value, true, false, true, false) || len == 0)) ThrowUnexpectedType(type); // throws
                }
        ...
        private class DeserializeItemsIterator : IEnumerator, IEnumerable
        {
            ...
            public bool MoveNext()
            {
                ...
                {
                    int bytesRead;
                    Current = model.DeserializeWithLengthPrefix(source, null, type, style, expectedField, resolver,
                        out bytesRead, out haveObject, context);
                }
        ...
        private object DeserializeCore(ProtoReader reader, Type type, object value, bool noAutoCreate)
        {
            ...
            TryDeserializeAuxiliaryType(reader, DataFormat.Default, Serializer.ListItemTag, type, ref value, true,
                false, noAutoCreate, false);
            return value;
        }
#endif
#if WINRT || COREFX
...
#if WINRT || COREFX
                TypeInfo constuctedListType =
 typeof(System.Collections.Generic.ICollection<>).MakeGenericType(types).GetTypeInfo();
#else
        ...
        internal static Type GetListItemType(TypeModel model, Type listType)
        {
            ...
            if (listType == model.MapType(typeof(string)) || listType.IsArray
                                                          || !model.MapType(typeof(IEnumerable))
                                                              .IsAssignableFrom(listType)) return null;
#endif
...
#else
            // more convenient GetProperty overload not supported on all platforms
            foreach (var indexer in listType.GetProperties(BindingFlags.Instance | BindingFlags.Public |
            ...
            switch (candidates.Count)
            {
                ...
                case 2:
                    if ((Type)candidates[0] != listType &&
                        CheckDictionaryAccessors(model, (Type)candidates[0], (Type)candidates[1]))
                        return (Type)candidates[0];
                    if ((Type)candidates[1] != listType &&
                        CheckDictionaryAccessors(model, (Type)candidates[1], (Type)candidates[0]))
        ...
        private static object CreateListInstance(Type listType, Type itemType)
        {
...
#endif
            {
                ...
                if (listType.IsInterface &&
                    ...
                    fullName.IndexOf("Dictionary") >= 0) // have to try to be frugal here...
                {
#if !NO_GENERICS
#if WINRT || COREFX
                    ...
                    if (finalType.IsGenericType && finalType.GetGenericTypeDefinition() == typeof(System.Collections.Generic.IDictionary<,>))
                    {
                        ...
                        concreteListType =
 typeof(System.Collections.Generic.Dictionary<,>).MakeGenericType(genericTypes);
                        handled = true;
                    }
#else
