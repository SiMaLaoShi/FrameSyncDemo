...
public class ToolMethod
{
    ...
    {
        var delAngle = _tarAngle - _objAngle;

        if (Mathf.Abs(delAngle) <= _speed) return _tarAngle;
        var angleSign = Math.Sign(delAngle);

        if (delAngle > 180 || delAngle < -180) angleSign *= -1;

        var angle = _objAngle + angleSign * _speed;
    ...
    {
        var delAngleZ = _objAngle - _tarAngle;
        var _fixangle = _objAngle;
        if (delAngleZ > 180)
            _fixangle = _objAngle - 360f;
        else if (delAngleZ < -180) _fixangle = _objAngle + 360f;
